...
import { DetailsStyle } from "../../orders/components/styles";
    ...
    <div className={classes.root} style={!order.ID ? { padding: 0, minHeight: 0 } : {}}>
      {isBilling ? (
        <ul className={classes.list}>
          <li className={classes.listHeadline}>{text("billing")}</li>
      ...
      {isShipping ? (
        <ul className={classes.list}>
          <li className={classes.listHeadline}>{text("shipping")}</li>
          <li className={classes.listLine}>
      ...
      {order.ID ? (
        <ul className={classes.list} style={isShipping ? {} : { width: 360 }}>
          ...
          {customerNote.length ? (
            <>
              <li className={classes.listHeadlineSecond}>{text("order_customer_note")}</li>
              <li className={classes.listLine}>
