...
import { HeaderStyle } from "./orderStyles";
  ...
  return (
    <div className={classes.root} style={!post.ID ? { padding: "35px 0 94px", height: 144 } : {}}>
      {loaderStatus ? (
        ...
        >
          Order #{post.ID} - {post.preview_date_format}
          {/* <br />
        ...
        <>
          ...
          <div>
            <span className={classes.resultText}>
              ...
              <br />
              {pluginData.settings.general?.dbOwnSearch === "on" ? text("check_internet_connection") : text("try_to_optimize_search")}
            </span>
        ...
        <>
          ...
          <div>
            {inputType === searchActions.inputTypes.SCAN ? (
              <span className={classes.resultText}>{text("order_scan_barcode")}</span>
            ) : (
              <span className={classes.resultText}>{text("order_type")}</span>
            )}
          </div>
        </>
      ) : (
